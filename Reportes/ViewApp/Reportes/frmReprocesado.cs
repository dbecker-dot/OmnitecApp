...
{
    public partial class frmReprocesado : Form
    {
        ...
        public frmReprocesado(frmMenuapp principal)
        {
            InitializeComponent();
            this.principal = principal;
        }

        private void frmReprocesado_Load(object sender, EventArgs e)
        {
            lbltituloform.Text = "PROCESADO ANUAL";
            cargacombos();
        }
        ...
        private void CargaGrilla()
        {
            try
            {
                if (filtraxano == true)
                {
                    dgvprod.DataSource = objrep.Procesadoxano();
                    dgvprod.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.Fill;
                }
                else
                {
                    dgvprod.DataSource = objrep.Procesadoxanomes();
                    dgvprod.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.Fill;
                }
            }
            catch (Exception)
        ...
        private void btnexportaexcel_Click(object sender, EventArgs e)
        {
            try
            {
                if (filtraxano == true)
                {
                    objrep.ExportarExcelProcesadoxano();
                }
                else
                {
                    objrep.ExportarExcelProcesadoxanomes();
                }
            }
            catch (Exception)
            {
